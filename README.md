...
- 普通聊天同时任务数：控制纯对话接口总共同时跑几个。
- 生图站同时任务数：每个账号在绘图站同时跑几个。
- 聊天生图同时任务数：每个账号在聊天站拿来生图时同时跑几个。
...
生图并发按账号单独计算。比如有 2 个启用账号，生图站填 `4`，就是生图站最多 `8` 个同时跑；聊天生图填 `4`，就是聊天生图最多 `8` 个同时跑。图生图会先尽量用所有账号的生图站，生图站都忙了再走聊天生图。
