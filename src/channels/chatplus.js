...
const IMAGE_RESULT_PROBE_TIMEOUT_SEC = 4;
...
const imageAuthFailureStats = new Map();
const activeImageCarLeases = new Map();
const imageConversationCarLeases = new Map();

function imageCarLeaseKey(account = {}, selected = {}) {
  const accountId = String(account.id || account.username || "").trim().toLowerCase();
  const carId = String(selected.carId || "").trim();
  const carType = String(selected.carType || "chatgpt").trim();
  return accountId && carId ? `${accountId}:${carType}:${carId}` : "";
}
...
function releaseConversationImageCar(conversationId) {
  const normalizedConversationId = String(conversationId || "").trim();
  if (!normalizedConversationId) return false;
  return releaseImageCarLease(imageConversationCarLeases.get(normalizedConversationId));
}

function imageCarIsLeased(account, selected) {
  pruneImageCarLeases();
  const key = imageCarLeaseKey(account, selected);
  return Boolean(key && activeImageCarLeases.has(key));
}
...
function rememberImageCarSuccess(accountId, carType, carId) {
  ...
  imageAuthFailureStats.delete(key);
  recentImageCarSuccessAt.set(key, Date.now());
  return hadCooldown;
}

function rememberImageAuthFailure(accountId, carType, carId, now = Date.now()) {
  const key = badCarKey(accountId, carType, carId);
  const previous = imageAuthFailureStats.get(key);
  const failureCount = previous && now - previous.lastAt <= IMAGE_AUTH_FAILURE_WINDOW_MS
    ? Math.min(IMAGE_AUTH_FAILURE_COOLDOWNS_MS.length, previous.failureCount + 1)
    : 1;
  const cooldownMs = IMAGE_AUTH_FAILURE_COOLDOWNS_MS[failureCount - 1];
  const next = { failureCount, lastAt: now, cooldownUntil: now + cooldownMs };
  imageAuthFailureStats.set(key, next);
  return next;
}
...
function imageTaskFailure(task = {}) {
  ...
  const finishReason = String(
    task.finish_details?.reason
      || task.finishDetails?.reason
      || task.result?.finish_details?.reason
      || task.result?.finishDetails?.reason
      || ""
  ).trim().toLowerCase();
  const cancelledByClient = ["client_stopped", "user_cancelled", "user_canceled"].includes(finishReason);
  if (!cancelledByClient && !["failed", "failure", "error", "cancelled", "canceled"].includes(status)) return null;
  ...
  return {
    status: cancelledByClient || ["cancelled", "canceled"].includes(status) ? "cancelled" : "failed",
    message: message || (cancelledByClient
      ? `上游已取消任务（${finishReason}）。`
...
function restoreSavedImageCarCooldowns(account = {}, channel = {}) {
  ...
  for (const savedCooldown of Object.values(savedImageCarCooldowns(account, channel))) {
    ...
    const failureCount = Math.max(0, Number(cooldown?.failureCount || 0));
    const updatedAt = Date.parse(cooldown?.updatedAt || "");
    if (failureCount > 0) {
      imageAuthFailureStats.set(badCarKey(accountId, carType, carId), {
        failureCount: Math.min(IMAGE_AUTH_FAILURE_COOLDOWNS_MS.length, failureCount),
        lastAt: Number.isFinite(updatedAt) ? updatedAt : Date.now(),
        cooldownUntil: until
      });
    }
  }
...
export class ChatplusClient {
  ...
  async findImageGenerationTask(conversationId, options = {}) {
    const normalizedConversationId = String(conversationId || "").trim();
    ...
    for (let page = 0; page < maxPages; page += 1) {
      ...
      const task = tasks.find((item) => imageTaskMatchesConversation(item, normalizedConversationId));
      if (task) return task;
  ...
  async imageGenerationTaskState(conversationId, options = {}) {
    ...
    if (!task) {
      try {
        task = await this.findImageGenerationTask(normalizedConversationId, options);
    ...
    const failure = imageTaskFailure(task);
    if (imageUrls.length || failure) releaseConversationImageCar(normalizedConversationId);
    return {
      taskId,
      task,
      imageUrls,
  ...
  preparedChatSession(session = {}) {
    return {
      ...
      snapshot: session.snapshot || this.sessionSnapshot()
    };
  }
  ...
  async enterCar(carId, carType, options = {}, sessionAlreadyLocked = false) {
    if (sessionAlreadyLocked) {
      await this.performEnterCar(carId, carType, options);
      return;
    }
  ...
  async selectCar(route, ignoredCarIds = new Set(), options = {}) {
    ...
    const candidatesBeforeLease = rankedCars(cars, selectionStrategy, selectionContext)
    ...
    if (!candidatesBeforeLease.length) throw new Error(`${route.name} 暂时没有可用车辆。`);
    const tierCandidates = candidatesBeforeLease.filter((item) => carMatchesTier(item.car, tier));
    if (!tierCandidates.length) throw new Error(`${route.name} 暂时没有可用的${carTierDisplayName(tier)}车位。`);
    const usableCarsBeforeLease = recheckProCars
      ? [
      ...
      : proCarsUnavailable
        ? tierCandidates.filter((item) => !item.car.isPro && !item.car.isUltra)
        : tierCandidates;
    if (!usableCarsBeforeLease.length) {
      throw new Error(`${route.name} 当前账号不能使用 PRO 车位，普通车位也暂时不可用。`);
    }
    const usableCars = options.imageGeneration === true
      ? usableCarsBeforeLease.filter((item) => !imageCarIsLeased(this.account, {
          carId: item.carId,
          carType: route.carType
        }))
      : usableCarsBeforeLease;
    if (!usableCars.length && options.imageGeneration === true) {
      const error = new Error("当前可用生图车位都在处理任务，请等待已有任务完成后再试。");
  ...
  async rememberCarCooldown(selected, retryAt, reason, message = "", metadata = {}) {
    ...
    const reason = isChatSubscriptionExpiredText(text)
      ...
      : isChatSubscriptionMissingText(text)
        ...
        : isCarPlanMismatchError(error)
          ? "car_plan_mismatch"
          : "car_auth";
    const failure = rememberImageAuthFailure(
      this.account?.id,
      selected?.carType,
      selected?.carId
    );
  ...
  async prepareChatSession(input = {}, ignoredCarIds = new Set(), maxAttempts = 5) {
    ...
    const requestOptions = {
      ...
      ...(input.imageGeneration === true ? { imageGeneration: true } : {}),
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
      ...
      try {
        ...
        let snapshot = null;
        let init = {};
        if (input.concurrentSubmit === true) {
          let reportLockStarted;
          ...
          init = route.key === "gpt"
            ? await measureTaskStage(input.taskStageRecorder, {
                ...
                carId: selected.carId,
                carType: selected.carType
              }, async () => sessionClient.loadInit(requestOptions))
            : {};
        }
        ...
        return {
          ...
          imageCarLease,
          snapshot: sessionClient.sessionSnapshot()
        };
      } catch (error) {
  ...
  async sendConversation(prompt, input = {}, ignoredCarIds = new Set()) {
    ...
    for (let attempt = 0; attempt < MAX_CHAT_CAR_ATTEMPTS; attempt += 1) {
      ...
      try {
        ...
        imageCarLease = session.imageCarLease || null;
        if (route.key === "grok") {
          const conversation = await runSubmitStep(() => submitClient.sendGrokConversation(prompt, requestInput, route, selected));
          confirmedConversationId = conversation.conversationId || "";
          bindImageCarLease(imageCarLease, confirmedConversationId);
        ...
        if (error?.code === "IMAGE_CAR_BUSY") throw error;
        if (imageCarLease && !confirmedConversationId) releaseImageCarLease(imageCarLease);
        if (
          requestInput.imageGeneration === true
          && !selected
          && leasedCarsSkipped > 0
          && /暂时没有可用的?.*(?:车辆|车位)/.test(String(error?.message || ""))
        ) {
          const busyError = new Error("当前可用生图车位都在处理任务，请等待已有任务完成后再试。");
          busyError.code = "IMAGE_CAR_BUSY";
        ...
        if (selected && error.imageCarBusy === true) {
          ignoredCarIds.add(selected.carId);
          leasedCarsSkipped += 1;
          const sessionKey = this.chatSessionKey(activeRoute || {});
          const cachedSession = this.concurrentChatSessions.get(sessionKey)?.session;
          if (cachedSession?.selected?.carId === selected.carId) {
            this.concurrentChatSessions.delete(sessionKey);
          }
          recordCarError(error.message || "当前车位正在生成图片");
          continue;
        }
  ...
  async withImageQuotaFallback(prompt, input, work) {
    if (input.imageGeneration === true) {
      const ignoredCarIds = new Set(
        (Array.isArray(input.excludedImageCarIds) ? input.excludedImageCarIds : [])
          .map((carId) => String(carId || "").trim())
          .filter(Boolean)
      );
      const quotaErrors = [];
      ...
      for (let attempt = 0; attempt < 5; attempt += 1) {
        ...
        try {
          ...
          if (Array.isArray(result?.imageUrls) && result.imageUrls.length) {
            releaseImageCarLease(conversation._imageCarLease);
          }
          return result;
        } catch (error) {
          if (conversation?._imageCarLease) releaseImageCarLease(conversation._imageCarLease);
  ...
  async waitForConversationImages(events, conversationId, timeoutSec, options = {}) {
    ...
    const timeoutMs = Math.max(5, Number(timeoutSec || this.config.waitTimeoutSec || DEFAULT_CHAT_HTTP_TIMEOUT_SEC)) * 1000;
    const deadline = Date.now() + timeoutMs;
    const probeTimeoutSec = () => Math.min(
      IMAGE_RESULT_PROBE_TIMEOUT_SEC,
    ...
    while (Date.now() < deadline) {
      ...
      try {
        ...
        let detail = null;
        try {
          detail = await this.conversationDetail(conversationId, {
  ...
  async getTask(externalId, context = {}) {
    ...
    const rawDetail = !geminiTask && (
      ...
      : detail;
    if (imageTaskState?.failure) {
      releaseConversationImageCar(externalId);
      return {
        externalId,
        status: imageTaskState.failure.status,
        imageCount: 0,
        imageUrls: [],
        errorMessage: imageTaskState.failure.message,
        raw: rawDetail
      };
    }
    if (imageUrls.length) {
