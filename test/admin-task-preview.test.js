...
test("改图接口默认等待最终结果，wait=0 才异步返回", () => {
  const routeStart = serverSource.indexOf('app.post("/v1/images/edits"');
  const routeEnd = serverSource.indexOf("\n});", routeStart);
  const routeSource = serverSource.slice(routeStart, routeEnd);

  assert.match(routeSource, /const waitForResult = request\.query\?\.wait !== "0"/);
  assert.match(routeSource, /wait: waitForResult/);
  assert.match(routeSource, /if \(!waitForResult \|\|/);
  assert.match(serverSource, /async function reserveImageRequestAdmission[\s\S]+verifyReady: true/);
});
...
test("并发面板明确显示聊天渠道和绘图站的独立容量", () => {
  assert.match(adminHtml, /queued: \["processing", "排队中"\]/);
  assert.match(adminHtml, /\{ label: "排队中", value: "queued" \}/);
  assert.match(adminHtml, /聊天渠道每账号最多 1 路/);
  assert.match(adminHtml, /聊天渠道并发（对话\+聊天生图）/);
  assert.match(adminHtml, /绘图站并发单独计算/);
  assert.doesNotMatch(adminHtml, /GPT 生图最多 2/);
  assert.doesNotMatch(adminHtml, /GPT 生图并发/);
});
